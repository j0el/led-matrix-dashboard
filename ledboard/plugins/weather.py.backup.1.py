...
class WeatherPlugin(BasePlugin):
    ...
    def render(self, width, height):
        ...
        if self.state.get("is_night"):
            # Night layout: moon + compact weather icon, no extra clutter.
            moon_phase = self.state.get("moon_phase")
            if moon_phase is not None:
                self._draw_moon(draw, width - 58, 22, 11, moon_phase)
            self._draw_condition_icon(draw, self.state.get("icon_code", "01n"), width - 38, 12)
        else:
            # Day layout: single larger weather icon, no moon.
        ...
        draw.text((118, stats_y), f"W{self.state['wind']}", font=font_stat, fill=(165, 165, 165))
